...
powershell "$f = \"Lightpack.sln\"; $sln = Get-Content $f; $guid = ($sln | select-string -Pattern 'prismatik-hooks32.*({[0-9A-Z\-]*}).*').Matches[0].Groups[1].Value; $sln = $sln -replace(\"($guid.*)x64\",'$1x86'); $guid = ($sln | select-string -Pattern 'prismatik-unhook32.*({[0-9A-Z\-]*}).*').Matches[0].Groups[1].Value; $sln = $sln -replace(\"($guid.*)x64\",'$1x86'); $guid = ($sln | select-string -Pattern 'offsetfinder.*({[0-9A-Z\-]*}).*').Matches[0].Groups[1].Value; $sln = $sln -replace(\"($guid.*)x64\",'$1x86'); $sln | Set-Content $f"
